...
%%%% Trends
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [] = Draw_FNC_Trends_offdiag(FC_input, domain, domain_ICN, Bar_range)
...
%     %     Linerange = ((0.5):1:(i+0.5));
%     %     axisposi  = (i-0.5).*ones(length(Linerange),1);
%     %
%     %     Linerange_col = ((0.5+i-2):1:(X_range+0.5));
%     %     axisposi_col  = (i-0.5).*ones(length(Linerange_col),1);
%     %
%     %     if (i ~= draw_idx)
%     %                 plot(Linerange, axisposi, 'k', 'LineWidth', 0.5);
%     %         plot(axisposi_col, Linerange_col, 'k', 'LineWidth', 0.5);
%     %     else
%     %         plot(Linerange, axisposi, 'k', 'LineWidth', 1.5);
%     %         plot(axisposi_col, Linerange_col, 'k', 'LineWidth', 1.5);
%     %     end
%     Linerange = ((0.5+i):1:(i+0.5+1));
%     axisposi  = (i-0.5).*ones(length(Linerange),1);
%     
%     Linerange = ((0.5):1:(i+0.5));
%     axisposi  = (i-0.5).*ones(length(Linerange),1);
%     
%     Linerange_col = ((0.5+i-2):1:(0.5+i-1));
%     axisposi_col  = (i-0.5).*ones(length(Linerange_col),1);
%     
%     if (i ~= draw_idx)
%         plot(Linerange, axisposi, 'k', 'LineWidth', 0.5);
...
for i = 1:(X_range+1)
    ...
    if i == 1
        plot([0.5 0.5], [0.5 53.5], 'k', 'LineWidth', 0.5);
        plot([0.5 1.5], [0.5 0.5], 'k', 'LineWidth', 0.5);
    elseif i == 54
        plot([53.5 53.5], [52.5 53.5], 'k', 'LineWidth', 0.5);
        plot([0.5 53.5], [53.5 53.5], 'k', 'LineWidth', 0.5);
    else
        Linerange = ((0.5+i-2):1:(0.5+i));
        axisposi  = (i+0.5-1).*ones(length(Linerange),1);
        
        plot(Linerange, axisposi, 'k', 'LineWidth', 0.5);
        plot(axisposi, Linerange, 'k', 'LineWidth', 0.5);
