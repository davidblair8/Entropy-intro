...
%%%% MRN
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [] = Draw_FC_Domain_update(FC_input, domain, domain_ICN, Bar_range)
...
num_hot = 50;
num_col = round(num_hot/ratio);
% hot map
colormap(rand(num_hot,3))
hot_map = colormap(hot);
hot_map_use = hot_map(end:-1:1,:);
% col map
colormap(rand(num_col,3))
hot_map = colormap(hot);
col_map_use = zeros(size(hot_map));
col_map_use(:,1) = hot_map(:,3);
col_map_use(:,2) = hot_map(:,2);
col_map_use(:,3) = hot_map(:,1);
